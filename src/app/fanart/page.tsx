...
export default function FanartGallery() {
  ...
  // ファンアート作品のみをフィルタリング
  const fanartItems = galleryItems.filter(item => item.category === 'fanart');
  ...
  const columnItems = Array.from({ length: columns }, (_, i) => {
    return fanartItems.filter((_, index) => index % columns === i);
  });
    ...
    <main className="min-h-screen bg-gradient-to-b from-gray-900 to-gray-800 text-white">
      <div className="container mx-auto px-4 py-8">
        <div className="mb-8">
          ...
          <h1 className="text-4xl font-bold text-center">Fan Art</h1>
          <p className="text-center text-gray-300 mt-4">ファンアート作品 ({fanartItems.length}件)</p>
        </div>
        
        {fanartItems.length === 0 ? (
          <div className="text-center text-gray-400 py-16">
            <p>ファンアート作品がまだありません</p>
          </div>
        ) : (
